...
public class graduateStudentCard {
        ...
        for (int i = 1; i < rowCount; i++) {
            ...
            if (data[i][0].equals(id)){
                ...
                for (int j = 1; j < colCount; j++){
                    infoArray[j][0] = data[0][j]; //place course names on the 0th column of infoArray
                    infoArray[j][1] = data[i][j]; //place actual grades on the 1st column of infoArray
                    infoArray[j][2] = Double.toString(averageArray[j-1]); //place course average grades on the 2nd column of infoArray
                }
    ...
    public String[][] getStudentInfo(String courseName){ return studentInfo(courseName); }
}
