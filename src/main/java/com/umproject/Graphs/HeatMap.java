...
public class HeatMap {
    ...
    public void CreateHeatMap(Pane root, String[][] gradeArray, double dimensionWidth, double dimensionHeight) {
        ...
        int maxCount = courseGrades.values().stream()
        ...
        for (String course : courseNames) {
            Map<Integer, Integer> gradeCounts = courseGrades.getOrDefault(course, Collections.emptyMap());
            ...
            if (minGrade > 0) {
                ...
                transparentData.nodeProperty().addListener((obs, oldNode, newNode) -> {
                    if (newNode != null) {
            ...
            //create a segment between the minimum grade and the maximum grade for each increment of 1
            for (int grade = minGrade; grade <= maxGrade; grade++) {
                int count = gradeCounts.getOrDefault(grade, 0);
                //get the color based on the count and the maximum count with function, if the count is greater than 0
    ...
    private String getColorIntensity(int count, int maxCount) {
        ...
        if (normalizedValue <= yellowStart) {
            ...
            green = (int) (255 - (normalizedValue - yellowStart) / (orangeStart - yellowStart) * 255);
        }
        else {
    ...
    private int findMinGrade(Map<Integer, Integer> gradeCounts) {return gradeCounts.keySet().stream().min(Integer::compare).orElse(0);}
    private int findMaxGrade(Map<Integer, Integer> gradeCounts) {return gradeCounts.keySet().stream().max(Integer::compare).orElse(10);}
