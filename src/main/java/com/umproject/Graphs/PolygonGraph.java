...
public class PolygonGraph {
    public static List<String> courses = Arrays.asList("JTE-234","ATE-003","TGL-013","PPL-239","WDM-974","GHL-823","HLU-200","MON-014","FEA-907","LPG-307","TSO-010","LDE-009","JJP-001","MTE-004","LUU-003","LOE-103","PLO-132","BKO-800","SLE-332","BKO-801","DSE-003","DSE-005","ATE-014","JTW-004","ATE-008","DSE-007","ATE-214","JHF-101","KMO-007","WOT-104");

    public static double[] averageGrades = {7.446601941747573, 8.008880994671403, 6.675041876046901, 7.058528428093646, 7.004854368932039, 6.769230769230769, 7.651282051282052, 6.983164983164984, 7.767169179229481, 7.607023411371237, 0, 6.951690821256038, 7.67741935483871, 7.4378330373001775, 7.547842401500938, 7.231793960923623, 0, 6.680320569902048, 7.451342281879195, 0, 7.145648312611012, 7.975806451612903, 6.703883495145631, 7.4966329966329965, 6.703703703703703, 6.9144385026737964, 7.8294849023090585, 6.5610859728506785, 7.371980676328502, 6.432885906040268};
    public static double[] averageGrades2 = {7.35729490748322,7.98848316139949,6.89798591779925,7.10288739697615,7.07248512635773,6.82435456579881,7.50537634408602,7.1413132470935,7.72217673707767,7.6767097865837,6.33677201026145,7.12641231373833,7.74903116642105,7.43283663555483,7.42175645434201,7.34042901588341,7.46580426832596,6.82195295016648,7.52207848916544,7.03247639320998,7.27443916816768,7.9633753616069,6.87626221276131,7.5093062605753,6.78128923093718,7.05933082255335,7.80863489984171,6.83800010916435,7.31482997652967,6.85382893946837};
    public static double[] averageGrades3 = {7.32281211301727,8.0,6.71075660240512,7.10102320927759, 6.89580087604077,6.66726055362304,7.46551652681431,7.01914759150483,7.80772053905871,7.63984508045933, 0 ,6.82442935353474,7.77856326849531,7.4,7.43608860670856,7.2, 0,6.7,7.47680877771967, 0,7.1,8.07379344643138,6.58957145574883,7.53987223629698,6.66505091334492,6.9,7.8,6.51171884053733,7.26046602034481,6.45537817737636};
    private void drawElements(Pane graphpane, double centerX, double centerY, double radius, double[] data1, double[] data2, Pane root) {
        ...
        double[][] cordFinal = generateTriacontagonVertices(radius, new double[]{centerX- Launcher.Distance(1920,30,root.getWidth()), centerY- Launcher.Distance(1080,5,root.getHeight())},35);
        double[][] cordOriginal = generateTriacontagonVertices(radius, new double[]{centerX,centerY},0);
        ...
        for(String i : courses) {
            double[] position = createPoint(graphpane, i, data1[courses.indexOf(i)], radius, new double[]{centerX, centerY}, Color.RED);
        ...
        for(String i : courses) {
            double[] position = createPoint(graphpane, i, data2[courses.indexOf(i)], radius, new double[]{centerX, centerY}, Color.BLUE);
            point[courses.indexOf(i)] = position;
        }
    ...
    public void showGraphInPopup(Stage stage, Pane root, double[] data1, int index) {
        ...
        double centerX = radius + Launcher.Distance(1920,10, root.getWidth()) + (additionalVerticalSpace / 2);
        double centerY = radius + Launcher.Distance(1080,10, root.getHeight()) + (additionalVerticalSpace / 2);

        //add the legend to the graph pane
        addLegendToPane(graphPane, root, 0, 0, 0);

        //create the triacontagon with the data1 array based on the index, and the Students predicate checkbox state
    ...
    public void addLegendToPane(@NotNull Pane graphPane, Pane root, int index, int id1, int id2) {
        ...
        legendBox.setStyle("-fx-background-color: white; -fx-border-color: black; -fx-border-width: 2px; -fx-border-radius: 5; -fx-background-radius: 5;");

        VBox legendItem1 = createLegendItem(index == 0 ?"Student Grade" : "Student " + id1, Color.RED, root);
        VBox legendItem2 = createLegendItem(index == 0 ?"Students averages" : "Student " + id2, Color.BLUE, root);
    ...
    public Pane draw(double centerX, double centerY, double radius, double[] data1, double[] data2, Pane root, int id1, int id2) {
