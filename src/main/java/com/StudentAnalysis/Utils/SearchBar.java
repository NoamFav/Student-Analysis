...
import com.StudentAnalysis.MainPage.Menu;
...
public class SearchBar {
    ...
    public SearchBar(Pane root, double x, double y, Stage stage) {
        ...
        Image loop =
                new Image(
                        Objects.requireNonNull(
                                getClass()
                                        .getResourceAsStream(
                                                "/com/StudentAnalysis/images/magnifier.png")));
        ...
        combinedList =
                Stream.concat(
                                Stream.concat(
                                        courses.stream(), studentIds.stream().map(String::valueOf)),
        ...
        // when clicked, search every instance created in the application, if there are, remove
        // them, and draw the appropriate page
        search.setOnAction(
                event -> {
    ...
    private void setupTextFieldListener() {
        textField
                .textProperty()
                .addListener((observable, oldValue, newValue) -> updateSuggestions(newValue));
    } // listener for the textfield

    private void setupEnterKeyListener(Stage stage) {
        // when pressing enter, search every instance created in the application, if there are,
        // remove them, and draw the appropriate page
