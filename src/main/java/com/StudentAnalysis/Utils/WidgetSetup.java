...
import com.StudentAnalysis.MainPage.Courses;
...
public class WidgetSetup {
    ...
    public WidgetSetup(Pane root) {
        ...
        down =
                new Image(
                        Objects.requireNonNull(
                                getClass()
                                        .getResourceAsStream(
                                                "/com/StudentAnalysis/images/DOWN.png")));
        up =
                new Image(
                        Objects.requireNonNull(
                                getClass()
                                        .getResourceAsStream(
                                                "/com/StudentAnalysis/images/UP.png")));
        constant =
                new Image(
                        Objects.requireNonNull(
                                getClass()
                                        .getResourceAsStream(
                                                "/com/StudentAnalysis/images/ConstantIcon.png")));
        ...
        sidePanel1.setHeight(rectangleHeight);
        sidePanel2.setHeight(rectangleHeight);
        sidePanel3.setHeight(rectangleHeight);
        sidePanel1.setWidth(rectangleWidth);
        sidePanel2.setWidth(rectangleWidth);
        sidePanel3.setWidth(rectangleWidth);
        ...
        root.getChildren()
                .addAll(
                        upIconViewForGreen,
                        downIconViewForGreen,
                        upIconViewForOrange,
                        downIconViewForOrange,
                        upIconViewForBlack,
                        downIconViewForBlack);
    }

    public static void updateLabelsAndIconsStudent(
            double greenValue,
            double orangeValue,
            double blackValue,
            double difference1,
        ...
        // change the label values for each side widget for a student, based on the index and the
        // predicate checkbox state
        // the infoLabel icons change up, down or constant based on the value of the corresponding
        // value
        ...
        if (index == 0) {
            if (Student.predicate.isSelected()) {
                ...
                difference1(
                        difference2, orangeInfoLabel, upIconViewForOrange, downIconViewForOrange);
    ...
    public static void updateLabelsAndIconsDataset(
            double greenValue,
            double orangeValue,
            double blackValue,
            double difference1,
        ...
        // change the label values for each side widget for a dataset, based on the index and the
        // predicate checkbox state
        // the infoLabel icons change up, down, or constant based on the value of the corresponding
        // value
        green.setText(String.format("%.2f", greenValue));
    ...
    public static void updateLabelsAndIconsCourse(
            double greenValue,
            double orangeValue,
            double blackValue,
            double difference1,
        ...
        // change the label values for each side widget for a course, based on the index and the
        // predicate checkbox state
        // the infoLabel icons change up down, or constant based on the value of the corresponding
        // value
        green.setText(String.format("%.2f", greenValue));
    ...
    private static void difference1(
            double difference, Label InfoLabel, ImageView upIconView, ImageView downIconView) {
        // set the icon up or down based on the value of the difference (positive or negative)
        if (difference >= 0) {
            InfoLabel.setText(
                    "up " + String.format("%.2f", Math.abs(difference)) + "% from others");
            InfoLabel.setGraphic(upIconView);
        } else {
            InfoLabel.setText(
                    "down " + String.format("%.2f", Math.abs(difference)) + "% from others");
    ...
    private static void difference2(
        ...
        if (difference >= 0) {
            InfoLabel.setText(
                    "up " + String.format("%.2f", Math.abs(difference)) + "% from graduates");
            InfoLabel.setGraphic(upIconView);
        } else {
            InfoLabel.setText(
                    "down " + String.format("%.2f", Math.abs(difference)) + "% from graduates");
            InfoLabel.setGraphic(downIconView);
        }
    }

    private static void difference3(
            double difference, Label InfoLabel, ImageView upIconView, ImageView downIconView) {
        // set the icon up or down based on the value of the difference (positive or negative)
        if (difference >= 0) {
            InfoLabel.setText(
                    "up " + String.format("%.2f", Math.abs(difference)) + "% from current");
            InfoLabel.setGraphic(upIconView);
        } else {
            InfoLabel.setText(
                    "down " + String.format("%.2f", Math.abs(difference)) + "% from current");
            InfoLabel.setGraphic(downIconView);
        }
    }
}
