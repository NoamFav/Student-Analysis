...
public class Chart {
    private static final String CURRENT_FILE_PATH =
            "src/main/resources/com/StudentAnalysis/CurrentGrades.csv";
    private static final String GRADUATE_FILE_PATH =
            "src/main/resources/com/StudentAnalysis/bugFreeGraduateGrades.csv";
    private static final String RATIO_FILE_PATH =
            "src/main/resources/com/StudentAnalysis/RatioPredictionRaw.csv";
    ...
    public void CreateGraph(Pane root, int index) {
        ...
        applyButton.setOnAction(
                e -> {
                    ...
                    if (courseNames != null) {
                        if (!SearchBar.combinedList.contains(selectedStudent)
                                || !SearchBar.combinedList.contains(secondSelectedStudent)) {
                            return;
                        }
                        if (index == 0) {
        ...
        fileChoiceBox.setOnAction(
                e -> {
                    if (fileChoiceBox.getValue().equals("GraduateGrades.csv"))
                        selectedFilePath = GRADUATE_FILE_PATH;
                    else if (fileChoiceBox.getValue().equals("CurrentGrades.csv"))
                        selectedFilePath = CURRENT_FILE_PATH;
                    else if (fileChoiceBox.getValue().equals("RatioPredictedGrades.csv"))
                        selectedFilePath = RATIO_FILE_PATH;
                    String[] courseNames = loadCourses();
                    if (courseNames != null) {
                        if (index == 0) {
                            updateLineChart(courseNames, pane, root);
    ...
    private String[] loadCourses() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(selectedFilePath))) {
            courses =
                    Arrays.copyOfRange(
                            br.readLine().split(","), 1, br.readLine().split(",").length);
        } catch (IOException e) {
    ...
    private void updateLineChart(String[] courseNames, VBox root, Pane mainPane) {
        ...
        if ("Highest to Lowest".equals(sortingOption)) {
            series.getData()
                    .sort(
                            Comparator.<XYChart.Data<String, Number>>comparingDouble(
                                            data -> data.getYValue().doubleValue())
                                    .reversed());
        } else if ("Lowest to Highest".equals(sortingOption)) {
            series.getData()
                    .sort(Comparator.comparingDouble(data -> data.getYValue().doubleValue()));
        }
        ...
        if (secondStudentGrades != null) {
            ...
            for (int i = 0; i < courseNames.length; i++) {
                secondSeries
                        .getData()
                        .add(new XYChart.Data<>(courseNames[i], secondStudentGrades[i]));
            }
    ...
    private void updateRadarChart(VBox root, Pane mainPane) {
        ...
        Pane pane =
                polygonGraph.draw(
                        mainPane.getWidth() / 2,
                        mainPane.getHeight() / 2.7,
                        Launcher.Distance(1080, 400, mainPane.getHeight()),
                        grades,
                        secondStudentGrades,
                        mainPane,
                        Integer.parseInt(selectedStudent),
                        Integer.parseInt(secondSelectedStudent));
        pane.setId(
                "radarChartPane"); // set an id to the graph pane so that it can be used in lookup

        // lookup if the radarChart is already in the root and if so, remove it and add the new
        // graph
        ...
        studentIdField
                ...
                .addListener(
                        (observable, oldValue, newValue) ->
                                updateSuggestions(newValue, studentIdField));
        secondStudentIdField
                .textProperty()
                .addListener(
                        (observable, oldValue, newValue) ->
                                updateSuggestions(newValue, secondStudentIdField));
