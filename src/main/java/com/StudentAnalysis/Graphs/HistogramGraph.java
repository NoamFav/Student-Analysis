...
public class HistogramGraph {
    ...
    public void createHistogram(String[] grades, Pane root) {
        ...
        // set tooltip for the bars and hover methods with listeners, as well as changing the style
        // of the bars.
        for (XYChart.Data<String, Number> data : series.getData()) {
            Tooltip tooltip =
                    new Tooltip("Grade: " + data.getXValue() + "\nCount: " + data.getYValue());
            data.nodeProperty()
                    .addListener(
                            (observable, oldNode, newNode) -> {
                                if (newNode != null) {
                                    ...
                                    newNode.setStyle(
                                            "-fx-bar-fill: #f3612d; -fx-border-color: black;"
                                                    + " -fx-border-width: 0.5;");
                                    Tooltip.install(newNode, tooltip);
                                    newNode.setOnMouseEntered(
                                            e ->
                                                    newNode.setStyle(
                                                            "-fx-bar-fill: rgba(173, 216, 230,"
                                                                + " 0.8); -fx-border-color: black;"
                                                                + " -fx-border-width: 0.5;"));
