...
public class LineGraph {
    ...
    private final double[] baselineData = {
        5, 5, 3, 5, 5, 4, 5, 4, 5, 4, 0, 4, 5, 4, 5, 4, 0, 4, 4, 0, 4, 5, 4, 5, 5, 5, 5, 5, 4, 5, 4
    };
    private final double[] floatingData = {
        5, 5, 7, 5, 4, 5, 5, 5, 5, 6, 0, 6, 5, 6, 5, 6, 0, 5, 6, 0, 6, 5, 5, 5, 5, 5, 5, 5, 5, 5
    };
    private final double[] graduateBaselineData = {
        6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6
    };
    private final double[] graduateFloatingData = {
        4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 3, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4
    };

    double[] graduateStudentPerformanceData = {
        7.35729490748322,
        7.98848316139949,
        6.89798591779925,
        7.10288739697615,
    ...
    private void createAndShowCharts(
        ...
        if (index == 0) {
            ...
            for (int i = 0; i < courseNames.length; i++) {
                baselineSeries
                        .getData()
                        .add(new XYChart.Data<>(courseNames[i], graduateBaselineData[i]));
            }
            for (int i = 0; i < courseNames.length; i++) {
                floatingSeries
                        .getData()
                        .add(new XYChart.Data<>(courseNames[i], graduateFloatingData[i]));
            }
        }
        ...
        for (XYChart.Data<String, Number> data : floatingSeries.getData()) {
            if (data.getNode() != null) {
                data.getNode()
                        .setStyle(
                                "-fx-bar-fill: #009966;-fx-effect: dropshadow(three-pass-box,"
                                        + " rgba(0,0,0,0.8), 10, 0, 0, 0);");
                data.getNode()
                        .setStyle(
                                data.getNode().getStyle()
        ...
        // set the averages based on the index and the predicate checkbox state and add the series
        // to the line chart
        if (index == 0) {
            for (int i = 0; i < courseNames.length; i++) {
                XYChart.Data<String, Number> data =
                        new XYChart.Data<>(
                                courseNames[i],
                                Student.predicate.isSelected()
                                        ? PolygonGraph.averageGrades3[i]
                                        : PolygonGraph.averageGrades[i]);
            ...
            for (int i = 0; i < courseNames.length; i++) {
                XYChart.Data<String, Number> data =
                        new XYChart.Data<>(courseNames[i], graduateStudentPerformanceData[i]);
                makeNodeInvisibleForZeroValue(data);
                studentSeries.getData().add(data);
            }
        }
        ...
        for (int i = 0; i < courseNames.length; i++) {
            XYChart.Data<String, Number> data =
                    new XYChart.Data<>(courseNames[i], studentGrades[i]);
            makeNodeInvisibleForZeroValue(data);
        ...
        title.setFont(
                Font.font(
                        "Georgia",
                        FontWeight.BOLD,
                        Launcher.Distance(1920, 20, dimensionWidth * 1.5)));
