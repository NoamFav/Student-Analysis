    ...
    public String[][][] studentInfo(String[][] data) {
        ...
        for (int i = 0; i < rowCount - 1; i++) {
            ...
            for (int j = 1; j < colCount; j++) {
                ...
                infoArray[i][j][1] =
                        data[i + 1][j]; // place actual grades on the 1st column of infoArray
            }
        }
        ...
        for (int i = 0; i < rowCount - 1; i++) {
            for (int j = 1; j < colCount; j++) {
                ...
                infoArray[i][j][5] =
                        Double.toString(
                                countersAllColumns[
                                        j - 1]); // place NG counts on the 5th column of infoArray
            }
        }
    }

    // Method to find the maximum and minimum grades of each course in the given CSV file and stores
    // them in the infoArray.
    public void printMaxMin() {
        ...
        for (int i = 0; i < rowCount - 1; i++) {
            for (int j = 1; j < colCount; j++) {
                infoArray[i][j][3] =
                        Double.toString(
                                minArray[j - 1]); // place min on the 3rd column of infoArray
                infoArray[i][j][4] =
                        Double.toString(
                                maxArray[j - 1]); // place max on the 4th column of infoArray
            }
        }
    }
