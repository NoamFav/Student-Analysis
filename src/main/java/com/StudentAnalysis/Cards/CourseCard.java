...
public class CourseCard {
    ...
    private final double[][] arrayWithoutNG =
            new double[rowCount - 1]
                    [colCount - 1]; // Initialize an array of student grades without NGs
    private final double[] sums =
            new double[rowCount - 1]; // An array to store the sum of grades that each student gets
    ...
    private final double[] NGs =
            new double[rowCount - 1]; // An array to store the number of NGs that each student gets
    ...
    public String[][][] courseInfo(String[][] data) {
        ...
        for (int i = 0; i < colCount - 1; i++) {
            ...
            infoArray[i][0][1] =
                    "Grade"; // store course names and the student's corresponding grade of the
            // course
            ...
            for (int j = 1; j < rowCount; j++) {
                ...
                infoArray[i][j][1] =
                        data[j][i + 1]; // place actual grades on the 1st column of infoArray
            }
        }
    ...
    // The method computes sums, averages and count NGs for each course in the given CSV file and
    // stores them in the infoArray.
    // The method also computes the number of NGs and grades for each student in the given CSV file
    // and stores them in the infoArray.
    public void processGrades(String[][] data) {
            ...
            for (int j = 1; j < colCount; j++) {
                ...
                // If it is a number, copy the value to arrayWithoutNG
                    ...
                    try {
                        ...
                        System.err.println(
                                "Error parsing number at position ["
                                        + i
                                        + "]["
                                        + j
                                        + "]: "
        ...
        // Store the average grade of each course, the number of NGs and grades, of each student to
        // infoArray
        for (int i = 0; i < colCount - 1; i++) {
            for (int j = 1; j < rowCount; j++) {
                infoArray[i][j][2] =
                        Double.toString(
                                averages[j - 1]); // place averages on the 2nd column of infoArray
                infoArray[i][j][3] =
                        Double.toString(grades[j - 1]); // place the number of grades on the 3rd
                // column of infoArray
                infoArray[i][j][4] =
                        Double.toString(NGs[j - 1]); // place the number of NGs on the 4th column
                // of infoArray
            }
        }
    }
