...
public class RegressionTreePoissonDeviance {
    ...
    private static class TreeNode {
        ...
        TreeNode() {
            this.isLeaf = false;
            this.splitThreshold =
                    Double.NaN; // Initialize with NaN to indicate no numerical threshold
    ...
    private void loadStudentInfo() throws IOException {
        try (BufferedReader br =
                new BufferedReader(
                        new FileReader(
                                "src/main/resources/com/StudentAnalysis/csv/StudentInfo.csv"))) {
    ...
    private void loadCurrentGrades() throws IOException {
        try (BufferedReader br =
                new BufferedReader(
                        new FileReader(
                                "src/main/resources/com/StudentAnalysis/csv/CurrentGrades.csv"))) {
    ...
    public void buildTree(
            ...
            Map<Integer, Map<String, Object>> studentInfo,
            Map<Integer, Map<String, Double>> currentGrades) {
    ...
    private TreeNode build(
        ...
        // create a leaf node if the current depth is greater than the max depth or if the number of
        // samples is less than the minimum samples
        int MAX_DEPTH = 10;
        if (currentDepth > MAX_DEPTH || studentIds.size() <= MIN_SAMPLES) {
            return createLeafNode(
                    ...
                    course,
                    currentGrades,
                    calculatePoissonDeviance(getGrades(studentIds, course, currentGrades)),
                    Average(getGrades(studentIds, course, currentGrades)),
                    studentIds.size());
        }
        ...
        boolean isNumerical =
                studentInfo.values().stream()
                        .anyMatch(map -> map.get(bestAttribute) instanceof Double);
        ...
        double poissonDevianceLeft =
                calculatePoissonDeviance(getGrades(leftSplitIds, course, currentGrades));
        double poissonDevianceRight =
                calculatePoissonDeviance(getGrades(rightSplitIds, course, currentGrades));
        ...
        if (poissonDevianceLeft <= MIN_POISSON_DEVIANCE) {
            node.leftChild =
                    createLeafNode(
                            ...
                            currentGrades,
                            poissonDevianceLeft,
                            Average(getGrades(leftSplitIds, course, currentGrades)),
                            leftSplitIds.size());
        } else {
            // otherwise recursively build the tree
            node.leftChild =
                    build(course, leftSplitIds, studentInfo, currentGrades, currentDepth + 1);
        }
        if (poissonDevianceRight <= MIN_POISSON_DEVIANCE) {
            node.rightChild =
                    createLeafNode(
                            ...
                            course,
                            currentGrades,
                            poissonDevianceRight,
                            Average(getGrades(rightSplitIds, course, currentGrades)),
                            rightSplitIds.size());
        } else {
            // otherwise recursively build the tree
            node.rightChild =
                    build(course, rightSplitIds, studentInfo, currentGrades, currentDepth + 1);
        }

        return node;
    }

    private TreeNode createLeafNode(
            List<Integer> studentIds,
            String course,
            Map<Integer, Map<String, Double>> currentGrades,
            double poissonDeviance,
    ...
    private String findBestPropertyAndThresholdForCourse(
            String course,
            Map<Integer, Map<String, Object>> studentInfo,
            List<Integer> subsetStudentIds) {
        ...
        for (String property : continuousProperties) {
            List<Double> thresholds =
                    getPossibleThresholds(property, studentInfo, subsetStudentIds);
            for (Double threshold : thresholds) {
                Map<String, Double> result =
                        checkScoreDifferenceByThreshold(
                                course, property, threshold, studentInfo, subsetStudentIds);
        ...
        for (String property : discreteProperties) {
            ...
            for (String value : uniqueValues) {
                Map<String, Double> result =
                        checkScoreDifferenceByProperty(
                                course, property, value, studentInfo, subsetStudentIds);
                Double poissonDeviance = result.get("PoissonDeviance");
                if (poissonDeviance != null && poissonDeviance < minPoissonDeviance) {
                    minPoissonDeviance = poissonDeviance;
        ...
        for (int i = 0; i < courses.indexOf(course); i++) {
            ...
            List<Double> thresholds =
                    getPossibleThresholds(gradeFeature, studentInfo, subsetStudentIds);
            for (Double threshold : thresholds) {
                Map<String, Double> result =
                        checkScoreDifferenceByThreshold(
                                course, gradeFeature, threshold, studentInfo, subsetStudentIds);
                Double poissonDeviance = result.get("PoissonDeviance");
                if (poissonDeviance != null && poissonDeviance < minPoissonDeviance) {
                    minPoissonDeviance = poissonDeviance;
    ...
    public Map<String, Double> checkScoreDifferenceByProperty(
            String course,
            String property,
    ...
    private Map<String, Double> checkScoreDifferenceByThreshold(
            String course,
            String property,
            double threshold,
            Map<Integer, Map<String, Object>> studentInfo,
    ...
    private List<Double> getPossibleThresholds(
            String property,
            Map<Integer, Map<String, Object>> studentInfo,
            List<Integer> subsetStudentIds) {
    ...
    private void addPreviousCourseGrades(String currentCourse) {
        ...
        for (Integer studentID : currentGrades.keySet()) {
            ...
            for (String course : courses) {
                // check NG count for each course, compare it with the current course's NG count and
                // skip the current course
                if (!course.equals(currentCourse)
                        && ngCounts.getOrDefault(course, 0) <= currentCourseNGCount) {
                    Double grade = grades.get(course);

                    studentInfo
                            .get(studentID)
                            .put("grade_" + course, Objects.requireNonNullElse(grade, Double.NaN));
                }
            }
        }
    }
    ...
    public Map<String, Double> calculateTotalPoissonDeviance(
            List<Double> group1, List<Double> group2) {
        ...
        double totalPoissonDeviance =
                (group1.size() * poissonDevianceGroup1 + group2.size() * poissonDevianceGroup2)
    ...
    public Pair<List<Integer>, List<Integer>> randomForestSplit(List<Integer> studentIds) {
        ...
        // the bootstrap sample is used to build the tree, the bootstrap sample is a random sample
        // with replacement
        // the OOB sample is used to evaluate the tree, the OOB sample is the data that was not
    ...
    public void runRegressionTree(
        ...
        for (int i = 0; i < treesNumber; i++) {
            ...
            List<Integer> filteredStudentIds =
                    new ArrayList<>(
                            studentIds.stream()
                                    .filter(
                                            id ->
                                                    currentGrades.containsKey(id)
                                                            && currentGrades
                                                                    .get(id)
                                                                    .containsKey(course))
                                    .toList());
            ...
            List<Integer> testIds =
                    filteredStudentIds.subList(numToSelect, filteredStudentIds.size());
    ...
    public void runRandomForest(
        ...
        // filter the student ids to only include students with a grade for the course
        List<Integer> filteredStudentIds =
                allStudentIds.stream()
                        .filter(
                                id ->
                                        currentGrades.containsKey(id)
                                                && currentGrades.get(id).containsKey(course))
                        .toList();

        // run the random forest for the specified number of trees

        for (int i = 0; i < numberOfTrees; i++) {
            Pair<List<Integer>, List<Integer>> splitResult =
                    randomForestSplit(new ArrayList<>(filteredStudentIds));
        ...
        double avgPoissonDeviance =
                forestPoissonDeviances.stream()
                        .mapToDouble(Double::doubleValue)
                        .average()
                        .orElse(Double.NaN);
        double avgAccuracy =
                forestAccuracies.stream()
                        .mapToDouble(Double::doubleValue)
                        .average()
                        .orElse(Double.NaN);
    ...
    private double predictGrade(
        ...
        while (!node.isLeaf) {
            ...
            if (isKnown) {
                ...
                // if the student is unknown, get the value for the split feature from unknown
                // student info
    ...
    public double predictForestGrade(
            int studentId,
            Map<Integer, Map<String, Object>> studentInfo,
            String course,
            boolean isKnown) {
    ...
    public double predictRegressionTreeGrade(
            int studentId,
            Map<Integer, Map<String, Object>> studentInfo,
            String course,
            boolean isKnown) {
    ...
    private void drawTree(
        ...
        double depthScalingFactor =
                Launcher.screenHeight / maxDepth; // Adjust vertical spacing based on maximum depth
        ...
        Text text =
                new Text(
                        rectX + Launcher.Distance(1920, 5, root.getWidth()),
                        rectY + Launcher.Distance(1080, 15, root.getHeight()),
                        nodeInfo);
        ...
        double widthOffset =
                (double) maxWidth
                        / Math.pow(2, depth + 1); // Corrected depth for offset calculation
    ...
    protected void buttonPrediction(
            boolean isSelected, String courseInput, int idInput, Label result2) throws IOException {
        List<Integer> studentIds =
                new ArrayList<>(RegressionTreePoissonDeviance.getCurrentGrades().keySet());
        if (isSelected) {
        ...
        if (isSelected) {
            prediction =
                    predictForestGrade(
                            idInput,
                            RegressionTreePoissonDeviance.getStudentInfo(),
                            courseInput,
                            true);
        } else {
            prediction =
                    predictRegressionTreeGrade(
                            idInput,
                            RegressionTreePoissonDeviance.getStudentInfo(),
                            courseInput,
                            true);
    ...
    private String getNodeInfo(TreeNode node) {
        ...
        if (node.isLeaf) {
            ...
            sb.append(
                            node.isNumerical
                                    ? ("Thr: " + String.format("%.4f", node.splitThreshold))
                                    : ("Cat: " + node.splitCategory))
                    .append("\n");
        }
    ...
    public void showTreeInPopup(
            Stage stage, String courseName, RegressionTreePoissonDeviance tree, Pane root) {
        // create the popup
        ...
        tree.drawTree(
                bestTree.get(courseName),
                (int) (Launcher.screenWidth / 2),
                50,
                0,
                (int) Launcher.screenWidth / 2,
                tree.calculateMaxDepth(bestTree.get(courseName)),
                pane,
                root);
        pane.setStyle(
    ...
    public static void main(String[] args) throws IOException {
        ...
        double accuracy =
                accuraciesForCourses.values().stream()
                        .filter(Double::isFinite)
                        .mapToDouble(Double::doubleValue)
                        .average()
                        .orElse(Double.NaN);
        double MSE =
                MSEForCourses.values().stream()
                        .filter(Double::isFinite)
                        .mapToDouble(Double::doubleValue)
                        .average()
                        .orElse(Double.NaN);
        ...
        double randomForestAccuracy =
                randomForestAccuracies.values().stream()
                        .filter(Double::isFinite)
                        .mapToDouble(Double::doubleValue)
                        .average()
                        .orElse(Double.NaN);

        double randomForestMSEs =
                randomForestMSE.values().stream()
                        .filter(Double::isFinite)
                        .mapToDouble(Double::doubleValue)
                        .average()
                        .orElse(Double.NaN);
        ...
        System.out.println(
                "(forest) grade for student 1001196 in course JTE-234: "
                        + tree.predictForestGrade(1001196, studentInfo, "JTE-234", true));
        System.out.println(
                "(forest) grade for student 1002438 in course TGL-013: "
                        + tree.predictForestGrade(1002438, studentInfo, "TGL-013", true));

        System.out.println(
                "(tree) grade for student 1001196 in course JTE-234: "
                        + tree.predictRegressionTreeGrade(1001196, studentInfo, "JTE-234", true));
        System.out.println(
                "(tree) grade for student 1002438 in course TGL-013: "
                        + tree.predictRegressionTreeGrade(1002438, studentInfo, "TGL-013", true));
