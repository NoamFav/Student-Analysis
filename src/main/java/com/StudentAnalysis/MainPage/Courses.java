...
import com.StudentAnalysis.AI.RegressionTreePoissonDeviance;
import com.StudentAnalysis.Cards.CourseCard;
import com.StudentAnalysis.Cards.graduateCourseCard;
import com.StudentAnalysis.Graphs.BoxPlot;
import com.StudentAnalysis.Graphs.HistogramGraph;
import com.StudentAnalysis.Graphs.PieChartGraph;
...
public class Courses {
    ...
    public Courses(Pane root, String input, Stage stage) {
        ...
        // switch between the two data sets on action: Current(index 0) and Graduate(index 1) and
    ...
    private void setData(int index) {
        ...
        // calculate the difference between the average of the course and the average of all
        // courses, based on the index and the checkbox state
        double difference1 = getDifference1(index, averages);
        double difference2 =
                (((1128 - validNumbersCount) - 598.1) / 598.1)
                        * 100; // calculate the difference between the number of NG on the course
        // and the average number of NG on all courses
        double difference3 =
                (((validNumbersCount) - 529.9) / 529.9)
                        * 100; // calculate the difference between the number of grades on the
        // course and the average number of grades on all courses

        // update the side Panel with the values and differences
        WidgetSetup.updateLabelsAndIconsCourse(
                averages,
                1128 - validNumbersCount,
                validNumbersCount,
                difference1,
                difference2,
                difference3,
        ...
        Image histogramImage =
                new Image(
                        Objects.requireNonNull(
                                getClass()
                                        .getResourceAsStream(
                                                "/com/StudentAnalysis/images/histogram.png")));
        ...
        Image treeImage =
                new Image(
                        Objects.requireNonNull(
                                getClass()
                                        .getResourceAsStream(
                                                "/com/StudentAnalysis/images/TreeIcon.png")));
        ...
        Image pieChartImage =
                new Image(
                        Objects.requireNonNull(
                                getClass()
                                        .getResourceAsStream(
                                                "/com/StudentAnalysis/images/PieChartIcon.png")));
        ...
        Image boxPLotsIcon =
                new Image(
                        Objects.requireNonNull(
                                getClass()
                                        .getResourceAsStream(
                                                "/com/StudentAnalysis/images/BoxPlotIcon.png")));
    ...
    private void LoadFile(
            String[][] graduateInfoArray, @NotNull ObservableList<String[]> dataList) {
        ...
        for (int i = 0; i < graduateInfoArray[0].length; i++) {
            ...
            tc.setComparator(
                    numericComparator); // add custom comparator to handle the String input with
            // numeric values
        }
